...
export default {
  data: new SlashCommandBuilder()
    .setName("chat-ai")
    .setDescription(
      "Rozmawiaj z AI z kontekstem kanału (tylko Ty widzisz odpowiedź)",
    )
    ...
    .addStringOption((option) =>
      option
        .setName("text")
        .setDescription("Twoja wiadomość")
        .setRequired(true),
    )
  ...
  run: async (interaction: ChatInputCommandInteraction) => {
    ...
    try {
        ...
        messages: [
          {
            role: "system",
            content:
              "Jesteś asystentem AI na Discordzie. Odpowiadaj krótko i po polsku.",
          },
          ...history,
          {
            role: "user",
            content: `[${interaction.user.username}]: ${prompt}`,
          },
        ],
      ...
      const response =
        ai.choices[0]?.message?.content || "Błąd generowania odpowiedzi.";
      ...
      console.error(error);
      await interaction.editReply({
        content: "Wystąpił błąd podczas komunikacji z Groq API.",
      });
    }
  },
};
